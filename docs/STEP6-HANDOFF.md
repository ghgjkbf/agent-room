...
## 〇.5、优化迭代增量（验收真实性等，2026-08-29）

- **交付核验**：`app/orchestrator/ceo.py` 的 `extract_claimed_paths`（提交付声称的文件路径，含中文）+ `_verify_claims`（核对工作区）；核验行注入验收员提示词；真实 LLM 模式下声称文件全部缺失 → 无视验收员硬打回（占位模式不启用，避免占位引用建议路径导致死循环）。**给执行者配 fs 白名单身份卡后，核验即全绿。**
- `subtasks.last_receipt` 列（最近验收/打回原因）→ 任务面板展示；`list_rooms` 网关工具；身份卡「一键：文件+技能」白名单按钮；janitor 遍历全部房间。
- pytest 47 例为回归底线。
