...
- **成员平权互聊**：所有成员广播互通——外部成员经网关发言首次能唤起内置成员接话（新增 reply_dispatch 总线监听器），内置 A↔B 广播也互相唤起；防死循环：连锁回复打 is_reply 标不再二次唤起，显式 @ 与排产单 dispatch 定向唤起不受限
- **按卡选人（噪声治理）**：身份卡新增 focus 发言领域字段——agent 广播只唤起 focus 关键词命中的成员，无命中由管家 B 兜底，外部成员默认静默（留空卡只响应 @/排产单）；出厂卡已预设领域关键词
- **白名单改道**：工具权限默认全开，仅 🔒 核心权限（shell.run / chat.archive / chat.delete / admin.* 三件）须身份卡显式勾选；网关与前端同规则，身份卡编辑器标注 🔒 并新增 focus 输入框
- **常用指令条**：输入框上方一排 chips——🧹归档清理 / 💾存记忆 / 🔍查记忆 / 🗑清完成任务 / ✂️删消息 / ⚠️清空记忆 / 📊房间状态；直调型（点即执行既有 confirm+API）与预填型（往输入框塞 @B/@A 指令模板补参数）双形态，中英双语；i18t 支持语义 key 直查
