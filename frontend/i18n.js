...
const I18N_EN = {
  ...
  '清空': 'Clear',
  /* 记忆面板 */
  '🗑 清空公共记忆': '🗑 Clear public memory',
  '清空本群聊的全部公共记忆？（私有记忆不受影响）': 'Clear ALL public memory of this group? (private memories untouched)',
  '已清空公共记忆：': 'Public memory cleared: ',
  '该条记忆无 id，无法删除': 'This item has no id and cannot be deleted',
  '已删除该条记忆。': 'Memory deleted.',
  '公共记忆': 'Public memory', '私有记忆': 'Private memory', '条': ' item(s)',
  ...
  '外观已恢复默认。': 'Appearance reset.',
  /* 归档与任务清理 */
  '🧹 立即归档清理（Agent B 执行）': '🧹 Archive now (run by Agent B)',
  '立即归档清理本群聊？（自上次归档以来的聊天将总结进公共记忆并清理原文）': 'Archive this group now? (chat since last archive will be digested into public memory, then pruned)',
  '归档完成：': 'Archive finished: ',
  '🧹 清空已完成任务': '🧹 Clear finished tasks',
  '（工作区暂无文件；先在「文件」面板上传或让 Agent 交付）': '(Workspace is empty; upload on the Files tab or have an agent deliver first)',
  '点击选择要引用的文件（可多选）': 'Click to toggle files to reference (multi-select)',
  '【引用工作区文件】': '[Referenced workspace files] ',
  '删除该任务记录？（群内消息保留）': 'Delete this task record? (room messages are kept)',
  '清空全部已结束（已完成/已作废）的任务记录？（群内消息保留）': 'Clear ALL finished (done/discarded) task records? (room messages are kept)',
  '已清空任务：': 'Tasks cleared: ',
  '任务未结束（先作废或等待完成）': 'Task not finished (discard it or wait for completion)',
  '删除失败': 'Delete failed',
  '（打回': ' (rejected ×', '次）': ')',
  '交付：': 'Delivery: ', '验收：': 'Review: ',
  '状态：': 'Status: ',
  '待派发': 'Queued', '执行中': 'Running', '已验收': 'Accepted', '已打回': 'Rejected',
  '待确认': 'Pending confirm', '已完成': 'Done', '已作废': 'Discarded',
  '💾 保存': '💾 Save',
  '删除': 'Delete',
...
const I18N_HELP_EN = `
      ...
      <div class="help-item"><b>Identity cards</b>: define a label, persona, responsibilities and a <b>tool allowlist</b> (fs.read / fs.write / fs.list, … — calls outside the list are rejected). Bind or rebind on the Members tab: external agents always have a "Bind identity" button, plus "Copy token" while unbound.</div>
